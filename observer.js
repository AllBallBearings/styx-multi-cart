...
(function () {
  ...
  // Top-of-IIFE log so we can tell from DevTools whether observer.js
  // even loaded on this URL. If you don't see this, the manifest match
  // is the problem (extension not reloaded, wrong TLD, or a path the
  // matches list doesn't cover).
  console.log("[Styx ATC] observer.js loaded on", location.href);
  ...
  console.log("[Styx ATC] page classification", {
  ...
  function hydrateCachesFromStorage() {
    try {
      chrome.storage.local.get(["mc.settings.v1", "mc.carts.v1"], (result) => {
        if (chrome.runtime.lastError) {
          console.warn("[Styx ATC] storage.get failed:", chrome.runtime.lastError.message);
          return;
        }
      ...
      console.warn("[Styx ATC] hydration error:", e);
    }
  }
  ...
  function installAtcDiagnostic() {
    document.addEventListener(
      ...
      (e) => {
        ...
        console.log("[Styx ATC] diagnostic — ATC-shaped click", {
          matchedBySelectors,
  ...
  function installAtcIntercept() {
    document.addEventListener(
      ...
      (e) => {
        ...
        console.log("[Styx ATC] click on ATC button", {
        ...
        if (_settingsCache.restoring) {
          console.log("[Styx ATC] restore in progress — letting click through");
          return;
        }
        ...
        if (btn.dataset.styxBypass === "1") {
          ...
          console.log("[Styx ATC] bypass flag set — letting click through");
          return;
        }

        if (!_settingsCache.interceptAtc) {
          console.log("[Styx ATC] intercept disabled in settings → falling through");
          return;
        }
        if (!Array.isArray(_cartsCache) || !_cartsCache.length) {
          console.log("[Styx ATC] no saved carts → falling through");
          return;
        }
        ...
        if (!item) {
          ...
          console.log("[Styx ATC] could not read ASIN → falling through. Ancestor chain:", chain);
          return;
        }

        console.log("[Styx ATC] intercepting click; opening picker", item);
        e.preventDefault();
