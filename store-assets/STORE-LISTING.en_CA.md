# Chrome Web Store — Listing Copy (Canadian English)

Canadian English-spelling variant of `STORE-LISTING.md` for the en_CA store listing. The "Notes for reviewer / permissions justification" section is intentionally omitted here — paste that straight from `STORE-LISTING.md` if the dashboard asks for it once (it isn't per-locale).
...
Shopping carts get mixed together quickly: gifts, groceries, work supplies, and items you are still comparing all compete for one checkout. Styx turns **Amazon Lists** into reusable carts, so each purchase can stay organised and separate.
