 ...
 *     productId: string,
 *     revoked: boolean        // true when StoreManager saw one of our product
 *                              // IDs with a revocationDate (e.g. a refund) and
 *                              // no other active entitlement covers the user.
 *                              // Distinct from an ordinary "not entitled"
 *                              // (nothing to see at all, e.g. never purchased
 *                              // or a subscription that simply lapsed) — see
 *                              // rule 3b below.
 *   }
 *
 ...
 *     UNLESS `native.revoked` is true (a refund), which always takes
 *     precedence and drops to free immediately regardless of any still-active
 *     window.
 *  4. Otherwise → free.
 *
...
export function nativeEntitlementToPatch(native, current, nowMs) {
  ...
  // A refund/revocation always wins over the still-active-premium floor below
  // — the whole point of the floor is to protect against downgrading on a
  // merely-absent or flaky read, and a revocation is neither: StoreManager
  // positively confirms the purchase was reversed.
  if (n.revoked === true) {
    return {
      tier: "free",
      premiumUntil: null,
