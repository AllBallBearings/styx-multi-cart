 ...
 * On the free tier only the first FREE_CART_LIMIT *custom* lists (in the given
 * order) are editable; the rest are "locked". Amazon's own default lists — the
 * Default List (Wish List, kind "default") and the Alexa Shopping List (kind
 * "alexa") — never count toward the limit and are always editable. Premium
 * unlocks every custom list.
 *
...
export function computeListAccess(lists, ent, nowMs = Date.now()) {
  ...
  const annotated = (Array.isArray(lists) ? lists : []).map((l) => {
    ...
    if (kind !== "custom") {
      return Object.assign({}, l, { kind, access: "editable" });
    }
    customSeen += 1;
...
//
// A saved cart can be mirrored into an Amazon wish list so it follows the
// user across devices. We stamp three fields on the cart when that link is
// established; they all default to null on carts that have never synced.

/**
 * Normalize the Amazon-list sync fields on every cart. Mutates in place and
 * returns the same array. Carts saved before the sync feature simply gain
 * three explicit nulls so the popup never has to probe for `undefined`.
 * Mirrored in background.js#readCarts — keep in sync.
 */
